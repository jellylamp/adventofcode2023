...
import {Trebuchet} from "./01-trebuchet";
...
treb7uchet`;
  ...
  const trebuchetOutput = new Trebuchet(input);
  expect(trebuchetOutput.trebuchetNumber).toEqual(142);
});
...
onenjhcd9`;
  ...
  const trebuchetOutput = new Trebuchet(input);
  expect(trebuchetOutput.trebuchetNumber).toEqual(55002);
  expect(trebuchetOutput.trebuchetNumberSpelledOut).toEqual(55093);
});
...
7pqrstsixteen`;
  ...
  const trebuchetOutput = new Trebuchet(input);
  expect(trebuchetOutput.trebuchetNumberSpelledOut).toEqual(369);
});

test("Test two numbers spelled out", () => {
  const input = `two1nine`;

  const trebuchetOutput = new Trebuchet(input);
  expect(trebuchetOutput.trebuchetNumberSpelledOut).toEqual(29);
});

test("Test two digits", () => {
  const input = `1two1nine1`;

  const trebuchetOutput = new Trebuchet(input);
  expect(trebuchetOutput.trebuchetNumberSpelledOut).toEqual(11);
});

test("Test same number multiple times", () => {
  const input = `eighteight9ninednvcqznjvfpreight`;

  const trebuchetOutput = new Trebuchet(input);
  expect(trebuchetOutput.trebuchetNumberSpelledOut).toEqual(88);
});


test("Test double digit no words", () => {
  const input = `fds1fds`;
...
test("Test double word no digit", () => {
  const input = `fdsonefds`;
...
test("Test double number", () => {
  const input = `eighteightwo`;

  const trebuchetOutput = new Trebuchet(input);
  expect(trebuchetOutput.trebuchetNumberSpelledOut).toEqual(82);
});
...
test("Test mixed same number", () => {
  const input = `eight88eight8eight8`;

  const trebuchetOutput = new Trebuchet(input);
  expect(trebuchetOutput.trebuchetNumberSpelledOut).toEqual(88);
});

test("Test addition with nothing", () => {
  const input = `eight88eight8eight8
  fdsdfsa
  twotwotwotwo`;

  const trebuchetOutput = new Trebuchet(input);
  expect(trebuchetOutput.trebuchetNumberSpelledOut).toEqual(110);
});

test("Test weird overlaps", () => {
  ...
  const trebuchetOutput = new Trebuchet(input);
  expect(trebuchetOutput.trebuchetNumberSpelledOut).toEqual(486);
});
...
seven687two1pdjbbprztwo`;
  ...
  const trebuchetOutput = new Trebuchet(input);
  expect(trebuchetOutput.trebuchetNumberSpelledOut).toEqual(2824); // 2824
});



test("Chunk 2", () => {
  const input = `eightrslvddpmonejfive4hhqbdq
xbhcqzx52eight52seventwo
fbgpj8fourthreecqngmbbvkktlbss
plckgsixeight6eight95bnrfonetwonej
1nine97rtrtxhqnone2
...
ninesevenkfnlhrznqseventhree9`;
  ...
  const trebuchetOutput = new Trebuchet(input);
  expect(trebuchetOutput.trebuchetNumberSpelledOut).toEqual(4382); // 2824
});

test("Chunk 3", () => {
  const input = `2fzbgjdvbnktwoonefivefiveqhz
seven28mxjxnzb
gqrznvtjeight9six277five
four3three
1kpqmrjctwolngk8
...
6jcmfivenine6fivefivethreetr`;
  ...
  const trebuchetOutput = new Trebuchet(input);
  expect(trebuchetOutput.trebuchetNumberSpelledOut).toEqual(5219); // 2824
});

// behold my shame, not using last index of to compare when there are multiple spelled out words and you are looking for the last number.
test("I cry", () => {
  const input = `five3ninektmsrxhsdrseveneight1onenine
onexvrsttmhfninetftbrhtwo1gkpbninekh`;

  const trebuchetOutput = new Trebuchet(input);
  expect(trebuchetOutput.trebuchetNumberSpelledOut).toEqual(78); // 2824
});
